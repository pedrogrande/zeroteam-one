...
| **Created** | 2026-04-20T22:31:38 |
| **Runs** | 1 completed / 2 total |

## Run 1 — Web Research Agent ✓ COMPLETED

*2026-04-20T22:31:38* · `glm-5.1:cloud` · `ca04b3eb...`

### Prompt

Create a template where my answers give you all the information you need to create an agent that aligns with the agent design principles below and you can create an Agno agent spec

### Response

Let me first check if there's any stored context about agent design principles or Agno specs that I should reference.No stored context is available, so I'll work from first principles. Below is a comprehensive **template** you can fill in — and when you give me the answers, I'll produce a fully-formed **Agno Agent Spec** that adheres to best-practice design principles.

---

# 🧩 Agent Design Questionnaire Template

Fill in each section as completely as you can. If a field isn't applicable, write **N/A**. When you're done, paste it back and I'll generate the Agno spec.
...
|---|----------|-------------|
| 1.1 | **Agent name** — a short, memorable identifier (e.g. `invoice_parser`, `support_triage`) | |
| 1.2 | **One-line purpose** — what does this agent *do* in a single sentence? | |
| 1.3 | **Detailed description** — expand on the purpose: who uses it, when, and why (2-4 sentences) | |
| 1.4 | **Agent type** — choose one: `conversational` / `workflow` / `autonomous` / `rag` / `tool-calling` | |
...
| # | Question | Your Answer |
|---|----------|-------------|
| 2.1 | **Model ID** — e.g. `gpt-4o`, `claude-3-5-sonnet`, `gpt-4o-mini` | |
| 2.2 | **Reasoning effort** — `low` / `medium` / `high` (default: `medium`) | |
| 2.3 | **Temperature** — 0.0 (deterministic) to 1.0 (creative). Default 0.0 for structured tasks. | |
| 2.4 | **Max tokens** — maximum response length (default: 4096) | |
...
| # | Question | Your Answer |
|---|----------|-------------|
| 3.1 | **Role description** — write the agent's persona in 2nd person ("You are a…") | |
| 3.2 | **Core constraints** — list hard rules the agent must always follow (e.g. "Never reveal internal IDs", "Always cite sources") | |
| 3.3 | **Tone & style** — e.g. `professional`, `friendly`, `technical`, `concise` | |
| 3.4 | **Failure mode** — what should the agent do when it doesn't know or can't do something? (e.g. "Ask a clarifying question", "Return a structured error") | |
| 3.5 | **Custom instructions** — any additional prompt fragments not covered above | |
...
| # | Question | Your Answer |
|---|----------|-------------|
| 4.1 | **Knowledge bases** — list any vector databases, file stores, or URLs the agent should search (name + description) | |
| 4.2 | **RAG strategy** — `default` / `agentic` / `simple` (default: `default`) | |
| 4.3 | **Max references** — how many source chunks to retrieve per query? (default: 5) | |
| 4.4 | **Agent memory** — `short-term` (session) / `long-term` (persistent across sessions) / `both` / `none` | |
| 4.5 | **Memory strategy** — how should memory be used? e.g. "Remember user preferences", "Recall past decisions" | |
...
| # | Question | Your Answer |
|---|----------|-------------|
| 5.1 | **Tools** — list each tool the agent needs (name + brief description). Example: `search_web: search the internet`, `send_email: send an email via Gmail` | |
| 5.2 | **Tool call limits** — max concurrent/sequential tool calls allowed (default: 5) | |
| 5.3 | **Tool selection strategy** — `auto` (agent decides) / `required` (always call) / `none` (never call) | |
| 5.4 | **External APIs / services** — any REST endpoints, SDKs, or databases to connect to? (URL + auth type) | |
| 5.5 | **Show tool calls** — should tool invocations be visible to the user? `true` / `false` | |

---

## 6. Session & Interaction Design

| # | Question | Your Answer |
|---|----------|-------------|
| 6.1 | **Session mode** — `single-turn` (one-shot) / `multi-turn` (conversational) | |
| 6.2 | **User message mode** — `text` / `voice` / `image` / `mixed` | |
| 6.3 | **Output format** — `markdown` / `json` / `plain-text` / `structured-object` (if structured, describe the schema) | |
| 6.4 | **Structured output schema** — if above is `structured-object`, paste or describe the JSON schema | |
| 6.5 | **Escalation path** — what happens when the agent hits a dead end? e.g. "Hand off to human", "Log ticket", "Re-ask user" | |
| 6.6 | **Markdown rendering** — render markdown in responses? `true` / `false` | |
...
| # | Question | Your Answer |
|---|----------|-------------|
| 7.1 | **Content filter** — restrict topics? e.g. "No medical advice", "No financial decisions without disclaimer" | |
| 7.2 | **PII handling** — `mask` / `block` / `allow` (should the agent process personally identifiable info?) | |
| 7.3 | **Approval workflow** — do certain actions require human sign-off? List them. | |
| 7.4 | **Audit logging** — log all interactions? `true` / `false` | |
...
| # | Question | Your Answer |
|---|----------|-------------|
| 8.1 | **Deployment target** — `local` / `docker` / `cloud` / `api-only` | |
| 8.2 | **Expected load** — approx. concurrent sessions (e.g. 10, 100, 1000) | |
| 8.3 | **Rate limits** — any upstream API rate limits to be aware of? | |
| 8.4 | **Team members** — list usernames/emails who should access this agent's configuration | |
...
| # | Question | Your Answer |
|---|----------|-------------|
| 9.1 | **Success metric** — how do you know this agent is working well? (e.g. "90% of tickets auto-resolved", "Average response < 5s") | |
| 9.2 | **Failure conditions** — what would make you shut it down? | |
| 9.3 | **Evaluation dataset** — do you have test inputs/expected outputs? If so, describe or link. | |

---

## How This Maps to Agno Agent Spec

When you return your answers, I'll translate them into an Agno agent spec that follows these **core design principles**:

| Principle | How I Enforce It |
|-----------|-----------------|
| 🎯 **Single Responsibility** | Each agent has one clear purpose (1.2–1.3) |
| 🧠 **Appropriate Reasoning** | Model + temperature + reasoning effort are tuned to task type (Section 2) |
| 🔧 **Minimal Tooling** | Only tools the agent actually needs are included (5.1), no kitchen-sink additions |
| 📏 **Structured Outputs** | Output schema enforced when precision matters (6.3–6.4) |
| 🛡️ **Guardrails by Default** | Safety, PII, and approval constraints are first-class (Section 7) |
| 🧩 **Composability** | Agents are designed to be composable — clean inputs/outputs, no hidden state |
| 📝 **Transparent Reasoning** | `show_tool_calls` and session design make the agent's behavior inspectable (5.5, 6.1) |
| 🔄 **Graceful Degradation** | Escalation paths and failure modes are explicit (3.4, 6.5) |
| 🧪 **Evaluability** | Success metrics and test sets are defined upfront (Section 9) |
| 💾 **State Awareness** | Memory strategy is intentional — not bolted on as an afterthought (Section 4) |

---

### Quick-Start Example (filled out)

Here's a tiny example so you can see the shape:

> **1.1** `support_triage`  
> **1.2** Classifies incoming customer support tickets by urgency and category.  
> **1.4** `workflow`  
> **2.1** `gpt-4o-mini`  
> **2.3** 0.0  
> **3.1** "You are a support ticket triage agent. Classify the user's issue and route it."  
> **3.2** Never auto-close tickets. Always assign a priority level.  
> **4.4** `short-term`  
> **5.1** `search_docs: search internal knowledge base`, `create_ticket: create a Jira ticket`  
> **6.1** `single-turn`  
> **6.3** `structured-object`  
> **6.4** `{"category": "str", "priority": "P0|P1|P2|P3", "summary": "str", "routing_team": "str"}`  
> **7.3** Any `P0` classification requires human confirmation before routing.  

---

**Copy the template, fill it in, and send it back — I'll produce a complete, production-ready Agno Agent Spec.**

---

## Run 2 — Web Research Agent ⚠️ ERROR

*2026-04-20T23:21:45* · `glm-5.1:cloud` · `75b30853...`

### Prompt

Here's my agent design principles. Can these add value to your template?
